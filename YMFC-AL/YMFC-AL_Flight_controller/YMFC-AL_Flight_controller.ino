...
void set_gyro_registers(){
  // Setup the MPU-6500
  if(eeprom_data[31] == 1){
    Wire.beginTransmission(gyro_address);  // Start communication with the gyro (0x68 or 0x69)
    Wire.write(0x6B);                      // PWR_MGMT_1 register
    Wire.write(0x00);                      // Set to 00000000 to wake up (same as MPU-6050)
    Wire.endTransmission();

    Wire.beginTransmission(gyro_address);
    Wire.write(0x1B);                      // GYRO_CONFIG register
    Wire.write(0x08);                      // Set to 00001000 (500dps full scale, same as MPU-6050)
    Wire.endTransmission();

    Wire.beginTransmission(gyro_address);
    Wire.write(0x1C);                      // ACCEL_CONFIG register
    Wire.write(0x10);                      // Set to 00010000 (+/- 8g full scale, same as MPU-6050)
    Wire.endTransmission();

    Wire.beginTransmission(gyro_address);
    Wire.write(0x1A);                      // CONFIG register (DLPF)
    Wire.write(0x03);                      // Set to 00000011 (DLPF ~44Hz, adjusted for MPU-6500 stability)
    Wire.endTransmission();

    // Optional: Disable strict GYRO_CONFIG check to avoid hang
    /*
    Wire.beginTransmission(gyro_address);
    Wire.write(0x1B);
    Wire.endTransmission();
