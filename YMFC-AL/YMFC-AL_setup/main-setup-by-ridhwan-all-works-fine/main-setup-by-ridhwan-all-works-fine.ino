...
void loop(){
  ...
  if(TWBR == 12 && clockspeed_ok){
    ...
    Serial.println(F("I2C clock speed is not set to 400kHz. (WARNING - Continuing anyway)"));
    // Do not set error
  }
  ...
  if(center_channel_1 < 900 || center_channel_1 > 2100) center_channel_1 = 1500;
  if(center_channel_2 < 900 || center_channel_2 > 2100) center_channel_2 = 1500;
  if(center_channel_3 < 900 || center_channel_3 > 2100) center_channel_3 = 1500;
  if(center_channel_4 < 900 || center_channel_4 > 2100) center_channel_4 = 1500;
  Serial.println(F(""));
  ...
  // Search for MPU-6500 at address 0x68 first (most common)
  Serial.println(F("Searching for MPU-6500 on address 0x68/104"));
  delay(500);
  byte whoami = search_gyro(0x68, 0x75);
  Serial.print(F("WHO_AM_I response: 0x"));
  Serial.println(whoami, HEX);
  if(whoami == 0x70 || whoami == 0x73){   // MPU-6500 WHO_AM_I can be 0x70 or 0x73
    Serial.println(F("MPU-6500 found on address 0x68"));
    type = 4;
    gyro_address = 0x68;
  }
  
  // If not found, search for MPU-6050 at 0x68
  if(type == 0){
    Serial.println(F("Searching for MPU-6050 on address 0x68/104"));
    delay(500);
    whoami = search_gyro(0x68, 0x75);
    Serial.print(F("WHO_AM_I response: 0x"));
    Serial.println(whoami, HEX);
    if(whoami == 0x68){   // MPU-6050
  ...
  // Search for MPU-6500 at address 0x69
  if(type == 0){
    Serial.println(F("Searching for MPU-6500 on address 0x69/105"));
    delay(500);
    whoami = search_gyro(0x69, 0x75);
    Serial.print(F("WHO_AM_I response: 0x"));
    Serial.println(whoami, HEX);
    if(whoami == 0x70 || whoami == 0x73){   // MPU-6500
      Serial.println(F("MPU-6500 found on address 0x69"));
      type = 4;
      gyro_address = 0x69;
    }
  }
  
  // Search for MPU-6050 at address 0x69
  if(type == 0){
  ...
  if(type == 0){
    Serial.println(F("No gyro device found!!! (WARNING - Continuing with defaults)"));
    type = 1; // Default to MPU-6050
    gyro_address = 0x68;
  }
  ...
  Serial.println(F("==================================================="));
  Serial.println(F("Don't move the quadcopter!! Calibration starts in 2 seconds"));
  delay(2000);
  ...
  if(receiver_check_byte == 0b00001111){
    ...
    Serial.println(F("Receiver channel verification failed!!! (WARNING - Continuing anyway)"));
  }
  delay(1000);
  if(gyro_check_byte == 0b00000111){
    ...
    Serial.println(F("Gyro axes verification failed!!! (WARNING - Continuing anyway)"));
  }
     
  //Always store the information in the EEPROM, even with warnings
  Serial.println(F(""));
  Serial.println(F("==================================================="));
  ...
  if(error == 1)Serial.println(F("EEPROM verification failed!!! (WARNING - Continuing anyway)"));
...
void start_gyro(){
  ...
  //Setup the MPU-6050 and MPU-6500
  if(type == 1 || type == 4){
...
void check_receiver_inputs(byte movement){
  ...
  if(trigger == 0){
    Serial.println(F("No stick movement detected in 30 seconds!!! (Using default assignment)"));
    // Use default assignments assuming standard wiring
    if(movement == 1){ // Throttle
      trigger = 3;
      pulse_length = 1000; // Assume low for default, no invert
      receiver_check_byte |= 0b00000100;
    }
    if(movement == 2){ // Roll
      trigger = 1;
...
void check_to_continue(){
  ...
  timer = millis() + 30000; // Timeout after 30 seconds
  while(continue_byte == 0 && timer > millis()){
  ...
  if(continue_byte == 0){
    Serial.println(F("No 'nose up' movement detected in 30 seconds!!! (Continuing anyway)"));
  }
  wait_sticks_zero();
}
...
void wait_sticks_zero(){
  byte zero = 0;
  timer = millis() + 30000; // Timeout after 30 seconds
  while(zero < 15 && timer > millis()){
    zero = 0;
    if(receiver_input_channel_1 < center_channel_1 + 50 && receiver_input_channel_1 > center_channel_1 - 50)zero |= 0b00000001; // Increased tolerance to +/-50
    if(receiver_input_channel_2 < center_channel_2 + 50 && receiver_input_channel_2 > center_channel_2 - 50)zero |= 0b00000010;
    if(receiver_input_channel_3 < center_channel_3 + 50 && receiver_input_channel_3 > center_channel_3 - 50)zero |= 0b00000100;
    if(receiver_input_channel_4 < center_channel_4 + 50 && receiver_input_channel_4 > center_channel_4 - 50)zero |= 0b00001000;
    delay(100);
  }
  if(zero < 15){
    Serial.println(F("Sticks not returned to center in 30 seconds!!! (Continuing anyway)"));
  }
}
...
void wait_for_receiver(){
  ...
  if(zero == 0){
    Serial.println(F("."));
    Serial.println(F("No valid receiver signals found!!! (Using defaults)"));
...
void register_min_max(){
  ...
  timer = millis() + 30000; // Timeout after 30 seconds for starting measurement
  while(millis() < timer){
    if(receiver_input_channel_1 > center_channel_1 + 50 || receiver_input_channel_1 < center_channel_1 - 50 ||
       receiver_input_channel_2 > center_channel_2 + 50 || receiver_input_channel_2 < center_channel_2 - 50 ||
       receiver_input_channel_3 > center_channel_3 + 50 || receiver_input_channel_3 < center_channel_3 - 50 ||
       receiver_input_channel_4 > center_channel_4 + 50 || receiver_input_channel_4 < center_channel_4 - 50) break;
    delay(250);
  }
  if(millis() >= timer){
    Serial.println(F("No initial stick movement detected in 30 seconds!!! (Using default min/max)"));
    low_channel_1 = 1000;
    low_channel_2 = 1000;
    low_channel_3 = 1000;
    low_channel_4 = 1000;
  ...
  timer = millis() + 30000; // Timeout after 30 seconds for returning to center
  while(zero < 15 && millis() < timer){
    zero = 0;
    if(receiver_input_channel_1 < center_channel_1 + 50 && receiver_input_channel_1 > center_channel_1 - 50)zero |= 0b00000001; // Increased tolerance
    if(receiver_input_channel_2 < center_channel_2 + 50 && receiver_input_channel_2 > center_channel_2 - 50)zero |= 0b00000010;
    if(receiver_input_channel_3 < center_channel_3 + 50 && receiver_input_channel_3 > center_channel_3 - 50)zero |= 0b00000100;
    if(receiver_input_channel_4 < center_channel_4 + 50 && receiver_input_channel_4 > center_channel_4 - 50)zero |= 0b00001000;
  ...
  if(zero < 15){
    Serial.println(F("Sticks not returned to center in 30 seconds!!! (Using measured values anyway)"));
  }
}
...
void check_gyro_axes(byte movement){
  ...
  if(trigger_axis == 0){
    Serial.println(F("No angular motion detected in 10 seconds!!! (Using default axis)"));
    if(movement == 1) trigger_axis = 0b00000001; // Default for roll
    if(movement == 2) trigger_axis = 0b00000010; // Default for pitch
    if(movement == 3) trigger_axis = 0b00000011; // Default for yaw
    gyro_check_byte |= (movement == 1 ? 0b00000001 : (movement == 2 ? 0b00000010 : 0b00000100));
  }
